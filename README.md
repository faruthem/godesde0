# 🚀 Go Desde Cero

[![Go Version](https://img.shields.io/badge/Go-1.25-blue)](https://go.dev/)
[![Go Actions](https://github.com/farithem/godesde0/actions/workflows/go.yml/badge.svg)](https://github.com/farithem/godesde0/actions)
[![License](https://img.shields.io/badge/License-MIT-green)](LICENSE)

Repositorio con ejemplos y ejercicios del curso de [Go desde cero](https://www.udemy.com/course/go-desde-cero/) en Udemy.

## 📚 Contenido

| Módulo | Descripción |
|--------|-------------|
| [variables](./variables) | Tipos de datos, declaraciones y conversiones |
| [iteraciones](./iteraciones) | Bucles y control de flujo |
| [teclado](./teclado) | Entrada de datos por teclado |
| [ejercicios](./ejercicios) | Ejercicios prácticos del curso |
| [mouredev](./mouredev) | Práctica con ejemplos básicos |

## 🛠️ Requisitos

- [Go](https://go.dev/dl/) 1.25 o superior

## 📦 Instalación

```bash
git clone https://github.com/farithem/godesde0.git
cd godesde0
go run main.go
```

## 📁 Estructura del Proyecto

```
godesde0/
├── main.go              # Punto de entrada
├── go.mod              # Módulo de Go
├── ejercicios/         # Ejercicios del curso
│   └── README.md
├── variables/          # Tipos de variables
│   └── README.md
├── iteraciones/        # Bucles y flujos
│   └── README.md
├── teclado/            # Entrada por teclado
│   └── README.md
└── mouredev/          # Práctica básica
    └── README.md
```

## ✅ Próximos Pasos

- [ ] Añadir tests unitarios
- [ ] Organizar con estructura `cmd/` y `pkg/`
- [ ] Añadir más ejercicios

---

## 🤝 Contribuir

Lee la guía de [CONTRIBUTING](CONTRIBUTING.md) para aprender cómo contribuir al proyecto.

---

⌨️ Curso impartido por [MoureDev](https://moure.dev/)